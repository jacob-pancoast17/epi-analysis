...
``` {r flu_burden, fig.align = "center", fig.caption = "Figure 1. Estimated ranges of annual flu and RSV burdens. Figure 1A was obtained from the CDC and shows flu burden from 2010-2025. Figure 1B was created using data from a CDC MMWR report on RSV from 2023 estimating the annual burden."}
...
flu_rsv_burdens <- 
  ...
  plot_annotation(
    title = "Disease Burden for Flu and RSV",
    theme = theme(
      plot.title = element_text(hjust = 0.5, vjust = -2, size = (16 * 1.3))
    )
  ) +
...
``` {r NWSS_sites, fig.align = "center", fig.cap = "Figure 2. United States NWSS Sites reporting RSV concentrations as of May 14, 2026. Source: CDC Wastewater Monitoring Program^9^", out.width = "600px"}
...
``` {r NREVSS_dash, fig.align = "center", fig.cap = "Figure 3. Percentage of positive RSV tests by state as of the week ending April 25, 2026. Source: CDC NREVSS Dashboard^10^", out.width = "800px"}
...
During this step a 7-day rolling average concentration was calculated for each data point to reduce noise and artifacts. All data cleaning, analyses, and visualizations were conducted using R (version 4.5.2; R Foundation)
...
``` {r visualize_exploratory, fig.align = "center"}
...
``` {r visualize_seasonal_offset, fig.align = "center"}
...
szn_lengths |> 
  kable(caption = "Table 1. Season lengths by site, including the averaged length.") |> 
...
peak_days |> 
  kable(caption = "Figure 2. Season peak days by site, including the average peak by season and site.") |> 
  kable_styling(position = "center")
...
peak_year |> 
  kable(caption = "Figure 3. Peak year for each site.") |> 
  kable_styling(position = "center")
...
Seasonal decomposition measures three core components of the time series:
...
such that $$ y_t $$ (Time series data) $$ = S_t + T_t + R_t $$
...
vt_rsv |> 
  ...
  kable(caption = "Figure S1. The population served by each participating NWSS site in Vermont.") |>
  kable_styling(position = "center")
